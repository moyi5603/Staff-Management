import type { EmployeeStatus, PositionStatus, ProjectDisplayStatus } from '../types';
import styles from './StatusBadge.module.css';

type Status = EmployeeStatus | ProjectDisplayStatus | PositionStatus | '正常' | '已撤销';
...
const config: Record<string, { bg: string; color: string }> = {
  ...
  未启动: { bg: 'var(--color-warning-bg)', color: 'var(--color-warning)' },
  已结束: { bg: '#f2f3f5', color: 'var(--color-text-secondary)' },
  已延期: { bg: 'var(--color-danger-bg)', color: 'var(--color-danger)' },
  已撤销: { bg: '#f2f3f5', color: 'var(--color-text-secondary)' },
  停用: { bg: '#f2f3f5', color: 'var(--color-text-secondary)' },
};
