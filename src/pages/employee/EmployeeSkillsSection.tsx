...
export function EmployeeSkillsSection({ skills, onChange, readOnly = false }: Props) {
  ...
  return (
    <div>
      {readOnly ? (
        <h4 className={styles.title}>技能列表</h4>
      ) : (
      ...
      {skills.length === 0 ? (
        <p className={styles.empty}>
          {readOnly ? '暂无个人技能。' : '暂无个人技能，点击「添加技能」选择。'}
        </p>
      ) : (
        <div className={styles.tagList}>
          {skills.map((s) => (
            <TagPill
              key={s}
              label={s}
              onRemove={readOnly ? undefined : () => onChange?.(skills.filter((x) => x !== s))}
            />
          ))}
        </div>
      )}
