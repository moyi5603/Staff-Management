...
- **部门负责人**（`leaderId`，`EmployeeSearchCombo` 边输边搜）即对外联络与 AI 查询用的部门联系人；数据来自员工管理 Mock。
- 详情展示：名称、上级、负责人、邮箱、联系电话（`phone`）、简介、文化、职能、绩效指标（`performanceIndicators` 文本）。
- 删除：需先转移员工（文案与 PRD 一致）。

### 员工

- 列表：关键词（姓名/工号/手机/邮箱）、部门/状态筛选、分页 20 条、导入三步向导 UI。
- 详情：顶部融合档案卡（摘要+基础信息）+ 4 Tab（证书/技能/兴趣/参与项目）均只读；改资料走「编辑」表单；手机号完整展示。
