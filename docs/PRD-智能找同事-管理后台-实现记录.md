...
- **Employee**：工号、姓名、手机/尾号、部门、岗位、生日、试用截止、微信绑定、最近登录、账号状态、技能、证书、荣誉、项目、兴趣、在职状态
...
- 员工字段：`accountStatus`、`createdAt`、`birthday`、`probationEndDate`、`wechatBound`、`lastLoginAt`；司龄按 `joinDate` 计算
- 列表列（与设计图一致）：员工姓名、工号、员工生日、入职时间、部门、手机号码、邮箱、试用截止、是否绑定微信小程序、最近登录时间、在职状态、账号状态（行内开关）、操作（修改 / 更多：查看详情、删除）
...
- **新增/编辑**：弹窗表单（主责部门、负责人、状态、日期、简介、参与部门、成员与角色）
