...
- 手机号完整展示（管理后台详情，不做掩码）
- 参与项目：只读表格（项目名称 / 我的角色 / 项目状态 / 开始时间 / 结束时间；日期随项目主数据同步）
- 个人技能/证书/兴趣：只读列表或标签展示；兴趣小组为 Mock 只读展示
- 增删改交互见 §6.1.1 编辑表单（技能 `SkillPickerModal`、证书 `CertificatePickerModal`、兴趣 `InterestPickerModal`）
